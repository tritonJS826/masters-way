...
import (
	...
	pbTraining "mw-general-bff/pkg/grpcAutoGenerated/training"
...
func (gs *GeminiFacade) GenerateTrainingByTestSessionId(ctx context.Context, payload *schemas.AIGenerateTrainingByTestTestSessionIdPayload, userId string) (*schemas.AIGenerateTrainingByTestTestSessionIdResponse, error) {
	...
	userIDRaw, _ := ctx.(*gin.Context).Get(auth.ContextKeyUserID)
	userID := userIDRaw.(string)
	authorizationRaw, _ := ctx.(*gin.Context).Get(auth.ContextKeyAuthorization)
	authorization := authorizationRaw.(string)

	headers := map[string]string{
		auth.ContextKeyAuthorization: authorization,
