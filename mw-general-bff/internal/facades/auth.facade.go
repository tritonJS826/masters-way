...
func newAuthFacade(authService *services.AuthService, notificationService *services.NotificationService, config *config.Config) *AuthFacade {
	return &AuthFacade{authService, notificationService, config}
}
...
func (af *AuthFacade) GetAuthCallbackFunction(ctx context.Context, provider, code, state string) (*openapiGeneral.MwServerInternalSchemasGetAuthCallbackFunctionResponse, error) {
	...
	if authCallbackFunctionResponse.IsAlreadyCreated {
		// authCallbackFunctionResponse.Url
		fmt.Println("url: ", authCallbackFunctionResponse.Url)

		// auth.ValidateJWT(tokenString string, secretSessionKey string)

		// ac.notificationService.CreateOwnEnabledNotifications(ctx, authCallback.UserUuid)
	}
...
func (af *AuthFacade) GetUserTokenByEmail(ctx context.Context, email string) (*openapiGeneral.MwServerInternalSchemasGetAuthCallbackFunctionResponse, error) {
	...
	if !authCallbackFunctionResponse.IsAlreadyCreated {
		...
		err = af.notificationService.CreateEnabledNotifications(ctx, claims.UserID)
		if err != nil {
			return nil, err
		}
	}
