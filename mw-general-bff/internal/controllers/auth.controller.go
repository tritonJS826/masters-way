...
func (ac *AuthController) InitiateTelegramLogin(ctx *gin.Context) {
	...
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "failed to initiate telegram login", "message": err.Error()})
		return
	}
...
func (ac *AuthController) GetLinkedUserByTelegramId(ctx *gin.Context) {
	...
	response, err := ac.authFacade.GetLinkedUser(ctx, tgID)
	if err != nil {
