...
const FILE_TO_LOG = "goalMigration_delete_metricUuids"
const log = (textToLog: string) => logToFile(`${(new Date()).toISOString()}: ${textToLog}`, FILE_TO_LOG);

/*
 * Delete metricUuids property to all goals
 */
const migrateGoals = async () => {
  ...
  log(`
    ...
    Migration goal:
    Delete "metricUuids" field from Goal collection
