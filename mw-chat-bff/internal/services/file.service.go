package services

import (
	openapiStorage "mw-chat-bff/apiAutogenerated/mw-storage"
	"mw-chat-bff/internal/schemas"

	"github.com/gin-gonic/gin"
)

type FileService struct {
	fileAPI *openapiStorage.APIClient
}

func NewFileService(fileAPI *openapiStorage.APIClient) *FileService {
	return &FileService{fileAPI}
}

func (cs *FileService) UploadFile(ctx *gin.Context) (*schemas.UploadFileResponse, error) {
	r := openapiStorage.ApiUploadFileRequest{
		ApiService: cs.fileAPI.FileAPI,
	}

	filePreviewRaw, _, err := cs.fileAPI.FileAPI.UploadFileStreamExecute(r, ctx.Request)
	if err != nil {
		return nil, err
	}

	return &schemas.UploadFileResponse{
		ID:      filePreviewRaw.Id,
		OwnerID: filePreviewRaw.OwnerId,
		Name:    filePreviewRaw.Name,
		Url:     filePreviewRaw.Url,
	}, nil
}
