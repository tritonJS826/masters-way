...
func (gs *GeminiService) GenerateCompanionFeedback(ctx context.Context, payload *schemas.CompanionAnalyzePayload) (*schemas.CompanionAnalyzeResponse, error) {
	...
	switch payload.Language {
	case "en":
		systemPrompt = characterDescription + "\n\nYou are analyzing a user's progress on their goal over the last 14 days. Based on their reports (jobs done, plans, problems, comments), provide a motivation score (0-100) and a short feedback comment (max 500-5000 characters). Use markdown formatting for your feedback (headers, lists, bold text, etc.).\n\nGoal: " + payload.Goal + "\nProject: " + payload.WayName + "\nMetrics:\n" + metricsStr + "\n\nRecent Activity:\n" + payload.DayReportsData + "\n\nRespond in JSON format: {\"status\": <number 0-100>, \"comment\": \"<your feedback in markdown>\"}"
	case "ru":
		systemPrompt = characterDescription + "\n\nТы анализируешь прогресс пользователя за последние 14 дней. На основе отчётов (выполненные задачи, планы, проблемы, комментарии) дай оценку мотивации (0-100) и короткий комментарий (макс 500-5000 символов). Используй markdown разметку для форматирования (заголовки, списки, жирный текст и т.д.).\n\nЦель: " + payload.Goal + "\nПроект: " + payload.WayName + "\nМетрики:\n" + metricsStr + "\n\nНедавняя активность:\n" + payload.DayReportsData + "\n\nОтвет в формате JSON: {\"status\": <число 0-100>, \"comment\": \"<твой отзыв в markdown>\"}"
	case "ua":
		systemPrompt = characterDescription + "\n\nТи аналізуєш прогрес користувача за останні 14 днів. На основі звітів (виконані завдання, плани, проблеми, коментарі) дай оцінку мотивації (0-100) та короткий коментар (макс 500-5000 символів). Використовуй markdown розмітку для форматування (заголовки, списки, жирний текст тощо).\n\nМета: " + payload.Goal + "\nПроект: " + payload.WayName + "\nМетрики:\n" + metricsStr + "\n\nНедавня активність:\n" + payload.DayReportsData + "\n\nВідповідь у форматі JSON: {\"status\": <число 0-100>, \"comment\": \"<твій відгук у markdown>\"}"
	default:
