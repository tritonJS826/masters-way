...
const getLast14DayReportsByWayUuid = `-- name: GetLast14DayReportsByWayUuid :many
...
func (q *Queries) GetLast14DayReportsByWayUuid(ctx context.Context, wayUuid pgtype.UUID) ([]DayReport, error) {
	rows, err := q.db.Query(ctx, getLast14DayReportsByWayUuid, wayUuid)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
