...
INSERT INTO users(
    ...
    $1, $2, $3, $4, $5, $6
) RETURNING uuid, name, email, description, created_at, image_url, is_mentor, telegram_chat_id
`
...
SELECT uuid, name, email, description, created_at, image_url, is_mentor, telegram_chat_id FROM users
WHERE email = $1
LIMIT 1
`
...
SELECT telegram_chat_id FROM users WHERE uuid = $1
`

func (q *Queries) GetUserTelegramChatID(ctx context.Context, userUuid pgtype.UUID) (pgtype.Int8, error) {
	row := q.db.QueryRow(ctx, getUserTelegramChatID, userUuid)
	var telegram_chat_id pgtype.Int8
