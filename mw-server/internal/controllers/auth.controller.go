...
func (ac *AuthController) InitiateTelegramLogin(ctx *gin.Context) {
	...
	pendingUser, err := ac.telegramService.CreatePendingTelegramUser(ctx, req.TelegramID, req.TelegramName, telegramCode, 10*time.Minute)
	if err != nil {
	...
	fmt.Printf("Pending user created: telegramId=%d, status=%s\n", pendingUser.TelegramID, pendingUser.Status)
...
type GetLinkedUserResponse struct {
	UserUuid string `json:"userUuid"`
	Email    string `json:"email"`
	Name     string `json:"name"`
}
...
func (ac *AuthController) GetLinkedUserByTelegramId(ctx *gin.Context) {
	...
	linkedUser, err := ac.telegramService.GetLinkedUserByTelegramId(ctx, tgID)
	log.Printf("GetLinkedUserByTelegramId: tgID=%d, err=%v, UserUuid.Valid=%v", tgID, err, linkedUser.UserUuid.Valid)
	if err != nil || !linkedUser.UserUuid.Valid {
		ctx.JSON(http.StatusNotFound, gin.H{"error": "user not linked"})
		return
	}
