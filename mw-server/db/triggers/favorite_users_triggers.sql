...
CREATE FUNCTION check_favorite_users_donor_user_uuid_limit() RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER check_favorite_users_donor_user_uuid_limit_trigger
BEFORE INSERT ON favorite_users
FOR EACH ROW
EXECUTE FUNCTION check_favorite_users_donor_user_uuid_limit();


-- favorite_users, acceptor_user_uuid
-- максимальное число лайков которое пользователь может получить от других пользователей
CREATE FUNCTION check_favorite_users_acceptor_user_uuid_limit() RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER check_favorite_users_acceptor_user_uuid_limit_trigger
BEFORE INSERT ON favorite_users
FOR EACH ROW
EXECUTE FUNCTION check_favorite_users_acceptor_user_uuid_limit();