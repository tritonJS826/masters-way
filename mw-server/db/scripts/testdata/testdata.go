...
func insertTestData(db *sql.DB) error {
	...
	unPrep := strings.Split(testData, ";")
	queries := unPrep[:len(unPrep)-1]
...
// Adds test data
// @Summary adds test data
// @Description adds test data for testing purposes
// @Tags test
// @Accept json
// @Produce json
// @Success 200
// @Router /test/data [post]
func AddTestData(db *sql.DB) gin.HandlerFunc {

	return func(ctx *gin.Context) {
		util.HandleErrorGin(ctx, insertTestData(db))
		ctx.JSON(http.StatusOK, gin.H{"status": "success"})
	}
}

// Resets all data
// @Summary resets all data
// @Description resets all data in the database for testing purposes
// @Tags test
// @Accept json
// @Produce json
// @Success 200
// @Router /test/data [delete]
func ResetData(q *db.Queries) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		util.HandleErrorGin(ctx, q.TruncateAllTables(ctx))
		ctx.JSON(http.StatusOK, gin.H{"status": "success"})
	}
}
...
// @Produce json
// @Success 200
// @Router /test/data [put]
func ResetAndAddTestData(q *db.Queries, db *sql.DB) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		util.HandleErrorGin(ctx, q.TruncateAllTables(ctx))
		util.HandleErrorGin(ctx, insertTestData(db))
		ctx.JSON(http.StatusOK, gin.H{"status": "success"})
