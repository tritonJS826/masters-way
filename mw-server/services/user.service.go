...
func GetPopulatedUserByIdParallel(db *dbb.Queries, ctx context.Context, userUuid uuid.UUID) (schemas.UserPopulatedResponse, error) {
	user, err := db.GetUserById(ctx, userUuid)

	ownWaysChan := make(chan []schemas.WayPlainResponse)
	mentoringWaysChan := make(chan []schemas.WayPlainResponse)
	favoriteWaysChan := make(chan []schemas.WayPlainResponse)
	wayCollectionsChan := make(chan []schemas.WayCollectionPopulatedResponse)
	tagsChan := make(chan []schemas.UserTagResponse)
	wayRequestsChan := make(chan []schemas.WayPlainResponse)
	favoriteForUsersUuidChan := make(chan []string)
	favoriteUsersChan := make(chan []schemas.UserPlainResponse)
	...
	go func() {
		dbMentoringWays, _ := db.GetMentoringWaysByMentorId(ctx, user.Uuid)
		mentoringWays := convertDbWaysToPlainWays(db, ctx, dbMentoringWaysToDbWays(dbMentoringWays))
		mentoringWaysChan <- mentoringWays
	}()
	go func() {
		dbFavoriteWays, _ := db.GetFavoriteWaysByUserId(ctx, user.Uuid)
		favoriteWays := convertDbWaysToPlainWays(db, ctx, dbFavoriteWaysToDbWays(dbFavoriteWays))
		favoriteWaysChan <- favoriteWays
	}()
	...
	go func() {
		...
		wayCollectionsChan <- wayCollections
	}()
	...
	go func() {
		...
		favoriteForUsersUuidChan <- favoriteForUsersUuid
	}()
	...
	go func() {
		...
		favoriteUsersChan <- favoriteUsers
	}()

	ownWays := <-ownWaysChan
	mentoringWays := <-mentoringWaysChan
	favoriteWays := <-favoriteWaysChan
	wayCollections := <-wayCollectionsChan
	tags := <-tagsChan
	wayRequests := <-wayRequestsChan
	favoriteForUsersUuid := <-favoriteForUsersUuidChan
	favoriteUsers := <-favoriteUsersChan
