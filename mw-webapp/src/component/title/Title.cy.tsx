...
describe("Title component", () => {
  ...
  it("render right value", () => {
    mountTitle();
    cy.get(getDataCy(TITLE_CY.dataCyTitleContainer)).should("contains.text", TITLE_CONTENT);
  ...
  it("should change to input on doubleClick and onChangeFinished triggered", () => {
    mountTitle();
    cy.get(getDataCy(TITLE_CY.dataCyTitleContainer)).dblclick();
    cy.get("input[type=\"text\"]")
      .type(ADDITIONAL_CONTENT)
      .type("{enter}");

    cy.get(getDataCy(TITLE_CY.dataCyTitleContainer)).should("contains.text", TITLE_CONTENT + ADDITIONAL_CONTENT);
