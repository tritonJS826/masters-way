import {
  CreateMetricRequest,
  DeleteMetricRequest,
  MwserverInternalSchemasMetricResponse,
  UpdateMetricRequest,
} from "src/apiAutogenerated/general";
import {metricService} from "src/service/services";

/**
 * Provides methods to interact with the metrics
 */
export class MetricService {

  /**
   * Create metric
   */
  public static async createMetric(requestParameters: CreateMetricRequest): Promise<MwserverInternalSchemasMetricResponse> {
    const jobDone = await metricService.createMetric(requestParameters);

    return jobDone;
  }

  /**
   * Update metric
   */
  public static async updateMetric(requestParameters: UpdateMetricRequest): Promise<MwserverInternalSchemasMetricResponse> {
    const updatedJobDone = await metricService.updateMetric(requestParameters);

    return updatedJobDone;
  }

  /**
   * Delete metric by UUID
   */
  public static async deleteMetric(requestParameters: DeleteMetricRequest): Promise<void> {
    await metricService.deleteMetric(requestParameters);
  }

}
