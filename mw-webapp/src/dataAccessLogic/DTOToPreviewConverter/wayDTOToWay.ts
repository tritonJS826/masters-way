...
import {MwGeneralBffInternalSchemasMetricTreeNode}
  from "src/apiAutogenerated/general/models/MwGeneralBffInternalSchemasMetricTreeNode";
 ...
 * Map tree metrics
 */
export const mapTreeMetrics = (metricA: MwGeneralBffInternalSchemasMetricTreeNode): Metric => {

  const mappedMetric = new Metric({
    children: metricA.children.map(mapTreeMetrics),
    doneDate: metricA.metric.doneDate ? new Date(metricA.metric.doneDate) : null,
    description: metricA.metric.description,
    isDone: metricA.metric.isDone,
    parentUuid: metricA.metric.parentUuid,
    uuid: metricA.metric.uuid,
  });

  if (metricA.children && metricA.children.length > 0) {
    mappedMetric.children = metricA.children.map(mapTreeMetrics); // Recursively map children
  }
 ...
 */
export const wayDTOToWay = (wayDTO: MwGeneralBffInternalSchemasWayPopulatedResponse, dayReports: DayReportsParams): Way => {
